...
alter table employees
    add constraint fk_employees_userId foreign key (userId) references users (id) on delete cascade on update restrict;

create index ix_items_itemCategoryId on items (itemCategoryId);
alter table items
    add constraint fk_items_itemCategoryId foreign key (itemCategoryId) references item_categories (id) on delete restrict on update restrict;

alter table momo_users
    add constraint fk_momo_users_userId foreign key (userId) references users (id) on delete cascade on update restrict;

create index ix_sales_employeeId on sales (employeeId);
alter table sales
    add constraint fk_sales_employeeId foreign key (employeeId) references employees (id) on delete cascade on update restrict;

create index ix_sale_details_itemId on sale_details (itemId);
alter table sale_details
    add constraint fk_sale_details_itemId foreign key (itemId) references items (id) on delete restrict on update restrict;

create index ix_sale_details_saleId on sale_details (saleId);
alter table sale_details
    add constraint fk_sale_details_saleId foreign key (saleId) references sales (id) on delete restrict on update restrict;

alter table students
    add constraint fk_students_userId foreign key (userId) references users (id) on delete cascade on update restrict;

create index ix_users_userRoleId on users (userRoleId);
alter table users
    add constraint fk_users_userRoleId foreign key (userRoleId) references user_roles (id) on delete restrict on update restrict;
